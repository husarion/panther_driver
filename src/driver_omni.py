...
def cmd_vel_callback(data):
    ...
    RK.FR_enc_speed = float(wheel_front_right) * float(RK.encoder_resolution) / (2 * math.pi) # tics/s
    RK.FL_enc_speed = float(wheel_front_left) * float(RK.encoder_resolution) / (2 * math.pi) # tics/s
    RK.RR_enc_speed = float(wheel_rear_right) * float(RK.encoder_resolution) / (2 * math.pi) # tics/s
    RK.RL_enc_speed = float(wheel_rear_left) * float(RK.encoder_resolution) / (2 * math.pi) # tics/s
...
def panther_driver():
    ...
    RK.robot_width = rospy.get_param('~robot_width', 0.67)
    RK.robot_length = rospy.get_param('~robot_length', 0.44)
    RK.wheel_radius = rospy.get_param('~wheel_radius', 0.1825)
    RK.encoder_resolution = rospy.get_param('~encoder_resolution', 38400)
