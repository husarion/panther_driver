...
class PantherMix(PantherKinematics):
    ...
    def forwardKinematics(self):
        ...
        wheel_front_right = (1/self.wheel_radius) * (self.lin_x +
                                                     (self.robot_width + self.robot_length * 2) * self.ang_z)
        wheel_front_left = (1/self.wheel_radius) * (self.lin_x -
                                                    (self.robot_width + self.robot_length * 2) * self.ang_z)
        wheel_rear_right = (1/self.wheel_radius) * \
            (self.lin_x + (self.robot_width) * self.ang_z)  # rad/s
        wheel_rear_left = (1/self.wheel_radius) * \
            (self.lin_x - (self.robot_width) * self.ang_z)
