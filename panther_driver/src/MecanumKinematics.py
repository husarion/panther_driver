...
class PantherMecanum(PantherKinematics):
    ...
    def forwardKinematics(self):
        # Mecanum:
        wheel_front_right = (1/self.wheel_radius) * (self.lin_x + self.lin_y +
                                                     (self.robot_width + self.robot_length) * self.ang_z)  # rad/s
        wheel_front_left = (1/self.wheel_radius) * (self.lin_x - self.lin_y -
                                                    (self.robot_width + self.robot_length) * self.ang_z)
        wheel_rear_right = (1/self.wheel_radius) * (self.lin_x - self.lin_y +
                                                    (self.robot_width + self.robot_length) * self.ang_z)
        wheel_rear_left = (1/self.wheel_radius) * (self.lin_x + self.lin_y -
                                                   (self.robot_width + self.robot_length) * self.ang_z)
